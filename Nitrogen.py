...
def Bethe_bloch(E_2, c):
    return 1/((2*6.89*(10**26))*(1/(E_2))*(np.log(E_2)+6.307-(np.log(c)))*(10**-19))
data = np.genfromtxt('Nitrogen.csv', delimiter = ',')
...
error_de= (-2*coeff[0]*(data[:,3]*10**-3))
coeff2 = np.polyfit(E, de, 2)
fitfunction2=np.poly1d(coeff2)
raw_data_figure = plt.figure()
raw_data_plot = raw_data_figure.add_subplot(111)
raw_data_plot.set_title('-dE/dx (J/m) against Energy(J) for Nitrogen', fontsize=12)
...
M_mid=M_E+h
ion_E=np.linspace(102.92, 106.92, 10000)
chi=[]
for i in ion_E:
    ...
    chi=np.append(chi, np.sum(((integral2-integral)/(unc_integral))**2))
