...
data = np.genfromtxt('Argon.csv', delimiter = ',')
...
def Bethe_bloch(E_2, c):
    ...
    return 1/(1.81*(10**27)*(1/(E_2))*(np.log(E_2)+6.307-(np.log(c)))*(10**-19))
...
raw_data_plot.set_title('Energy(J) against thickness(m) for Argon', fontsize=12)
...
M_mid=M_E+h
ion_E=np.linspace(100, 239.26, 10000)
chi=[]
for i in ion_E:
